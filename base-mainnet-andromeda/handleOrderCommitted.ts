...
export function handleOrderCommitted(event: OrderCommittedEvent): void {
  // const orderId = event.params.marketId.toString() + '-' + event.params.accountId.toString() + event.transaction.hash.toHexString();
  ...
  orderCommitted.transactionHash = event.transaction.hash;
