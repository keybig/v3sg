...
export function handleOrderSettled(event: OrderSettledEvent): void {
  ...
  if (!event.params.newSize.isZero()) {
    // average entry price and unrealized pnl
    let absOldSize = positionEntity.size.abs();
    let absSizeDelta = event.params.sizeDelta.abs();
    let absNewSize = event.params.newSize.abs();

    // Update averageEntryPrice using absolute values
    positionEntity.averageEntryPrice = absOldSize
      .times(positionEntity.averageEntryPrice)
      .plus(absSizeDelta.times(event.params.fillPrice))
      .div(event.params.newSize.abs());

    // Calculate unrealizedPnl based on the current market price
    let currentMarketPrice = event.params.fillPrice; // get market price
    if (event.params.newSize.gt(BigInt.zero())) {
      positionEntity.unrealizedPnl = event.params.fillPrice
